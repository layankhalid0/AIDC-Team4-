...
CPU model (API labs): `Qwen/Qwen3-8B`.
...
GPU model (measurement + serving): `Qwen/Qwen3-8B`
(fp16 about 3.1 GB of weights). AWQ build: `Qwen/Qwen3-8B-AWQ`.
...
- Qwen3-8B, Hermes-3: `--enable-auto-tool-choice --tool-call-parser hermes`
...
`Qwen/Qwen3-8B` with the `hermes` parser.
